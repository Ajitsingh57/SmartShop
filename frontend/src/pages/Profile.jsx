...
const Profile = () => {
  ...
  const handlePhoneUpdate = async (e) => {
    ...
    try {
      ...
      setError(err?.message || "Unable to update phone number.");
    } finally {
      setLoading(false);
    }
  };
              ...
              {addingPhone ? (
                ...
                <div>
                  ...
                  <button
                    type="button"
                    onClick={handleAddPhone}
                    className="mt-3 rounded-lg border border-[var(--app-accent-border)] bg-[var(--app-accent-soft)] px-3 py-2 text-xs font-semibold text-[var(--app-accent)] transition hover:bg-[var(--app-accent)] hover:text-white"
                  >
                    Add Phone
                  </button>
                </div>
